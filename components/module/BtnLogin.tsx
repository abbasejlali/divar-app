"use client";
import RefreshTokenValidatClient from "@/app/actionserver/RefreshTokenValidatClient";
import ValidationUserClient from "@/app/actionserver/ValidationUserClient";
...
function BtnLogin() {
  ...
  useEffect(() => {
    const data = async () => {
      await RefreshTokenValidatClient();
    };
  ...
  return (
    <>
      <Link
        href={`${validation ? "/dashboard" : "/login"}`}
